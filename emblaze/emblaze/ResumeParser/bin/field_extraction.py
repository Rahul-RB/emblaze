...
def candidate_name_extractor(input_string, nlp):
    ...
    doc_persons = [x for x in doc_entities if x.label_ == 'PERSON']
    doc_persons = [x for x in doc_persons if len(x.text.strip().split()) >= 2]
    doc_persons = [x.text.strip() for x in doc_persons]
